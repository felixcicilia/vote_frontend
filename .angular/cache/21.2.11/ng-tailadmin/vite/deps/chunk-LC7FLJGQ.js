...
__export(Math_exports, {
  ...
  resolveLocationOnPath: () => resolveLocationOnPath,
  round: () => round,
  sin: () => sin,
...
function getPointOnCubicCurve(pointA, pointB, controlPointA, controlPointB, position) {
  let s2 = 1 - position;
  let x = s2 * s2 * s2 * pointA.x + 3 * s2 * s2 * position * controlPointA.x + 3 * s2 * position * position * controlPointB.x + position * position * position * pointB.x;
  let y = s2 * s2 * s2 * pointA.y + 3 * s2 * s2 * position * controlPointA.y + 3 * s2 * position * position * controlPointB.y + position * position * position * pointB.y;
  return { x, y };
}
...
function add2(date, unit, count, utc, timezone) {
...
var ZoomableContainer = class extends Container {
  constructor() {
    ...
    this._downX = 0;
    this._downY = 0;
    this.contents = this.children.push(Container.new(this._root, {
      width: p100,
      height: p100,
      x: 0,
      y: 0,
...
ZoomableContainer.className = "ZoomableContainer";
ZoomableContainer.classNames = Container.classNames.concat([ZoomableContainer.className]);

// node_modules/@amcharts/amcharts5/.internal/core/render/ZoomTools.js
var ZoomTools = class extends Container {
  constructor() {
    super(...arguments);
    this.homeButton = this.children.push(Button.new(this._root, { width: 35, height: 35, themeTags: ["home"] }));
    this.plusButton = this.children.push(Button.new(this._root, { width: 35, height: 35, themeTags: ["plus"] }));
    this.minusButton = this.children.push(Button.new(this._root, { width: 35, height: 35, themeTags: ["minus"] }));
  }
  _afterNew() {
    super._afterNew();
...
//# sourceMappingURL=chunk-LC7FLJGQ.js.map
