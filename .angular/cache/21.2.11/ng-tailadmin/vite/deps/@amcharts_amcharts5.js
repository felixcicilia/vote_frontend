  ...
  */
  ...
  render(root) {
    ...
    if (deferredGhostLayers.length) {
      deferredGhostLayers.sort((a2, b) => a2 - b);
  ...
  getObjectAtPoint(point5) {
    const data = this._ghostLayer.getImageArray(point5);
...
var GhostLayer = class {
  ...
  getImageArray(point5) {
    if (!this.imageArray) {
      this.imageArray = this.context.getImageData(0, 0, this._width, this._height).data;
    }
    const data = this.imageArray;
    const x2 = Math.round(point5.x * this._resolution);
    const y2 = Math.round(point5.y * this._resolution);
...
var Root = class _Root {
  ...
  documentPointToRoot(point5) {
    ...
    return {
      x: (point5.x - rect.left) * scaleWidth,
      y: (point5.y - rect.top) * scaleHeight
    };
  }
  /**
  ...
  rootPointToDocument(point5) {
    ...
    const scaleHeight = size.height / rect.height;
    return {
      x: point5.x / scaleWidth + rect.left,
      y: point5.y / scaleHeight + rect.top
    };
  }
  /**
   * @ignore
  ...
  _updateComputedStyles() {
    ...
    const newFonts = keys(styles).flatMap((k2) => {
      var _a;
      return typeof k2 === "string" && k2.startsWith("font") ? [(_a = styles[k2]) === null || _a === void 0 ? void 0 : _a.toString()] : [];
    });
...
Modal.classNames = Entity.classNames.concat([Modal.className]);
...
// node_modules/@amcharts/amcharts5/.internal/core/render/ConfirmButton.js
var ConfirmButton = class extends Button {
  _afterNew() {
    super._afterNew();
    this.addTag("confirm");
    if (supports("keyboardevents")) {
...
ConfirmButton.classNames = Button.classNames.concat([ConfirmButton.className]);
...
Polygon.classNames = Graphics.classNames.concat([Polygon.className]);
...
Ellipse.classNames = Graphics.classNames.concat([Ellipse.className]);
...
Star.className = "Star";
Star.classNames = Graphics.classNames.concat([Star.className]);
...
var HeatLegend = class extends Container {
  constructor() {
    super(...arguments);
    this.labelContainer = this.children.push(Container.new(this._root, {}));
    this.markerContainer = this.children.push(Container.new(this._root, {}));
    this.startLabel = this.labelContainer.children.push(Label.new(this._root, { themeTags: ["start"] }));
    this.endLabel = this.labelContainer.children.push(Label.new(this._root, { themeTags: ["end"] }));
    this.markers = this.addDisposer(new ListTemplate(Template.new({}), () => RoundedRectangle._new(this._root, {
...
HeatLegend.className = "HeatLegend";
HeatLegend.classNames = Container.classNames.concat([HeatLegend.className]);
...
var Legend = class extends Series {
  constructor() {
    super(...arguments);
    this.itemContainers = this.addDisposer(new ListTemplate(Template.new({}), () => Container._new(this._root, {
    ...
    this.markers = this.addDisposer(new ListTemplate(Template.new({}), () => Container._new(this._root, {
    ...
    this.valueLabels = this.addDisposer(new ListTemplate(Template.new({}), () => Label._new(this._root, {
      themeTags: mergeTags(this.valueLabels.template.get("themeTags", []), ["legend", "label", "value"])
    }, [this.valueLabels.template])));
    this.markerRectangles = this.addDisposer(new ListTemplate(Template.new({}), () => RoundedRectangle._new(this._root, {
...
Legend.classNames = Series.classNames.concat([Legend.className]);
...
// node_modules/@amcharts/amcharts5/.internal/core/render/OrthogonalLine.js
var OrthogonalLine = class _OrthogonalLine extends Graphics {
  _beforeChanged() {
    super._beforeChanged();
    if (this.isDirty("points")) {
      this._clear = true;
  ...
  _changed() {
    ...
    if (this._clear) {
      ...
      points = _OrthogonalLine.makeOrthogonal(points);
      ...
      for (let i = 1; i < points.length; i++) {
        ...
        const directionChanged = next && (isHorizontal && next.y !== curr.y || isVertical && next.x !== curr.x);
        if (cornerRadius > 0 && next && directionChanged) {
          if (isVertical) {
            ...
            display.arcTo(curr.x, curr.y, curr.x + cornerRadius * signX, curr.y, cornerRadius);
            ...
            display.arcTo(curr.x, curr.y, curr.x, curr.y + cornerRadius * signY, cornerRadius);
          }
        } else {
...
OrthogonalLine.className = "OrthogonalLine";
OrthogonalLine.classNames = Graphics.classNames.concat([OrthogonalLine.className]);

// node_modules/@amcharts/amcharts5/.internal/core/render/Link.js
var Link = class extends Container {
  constructor() {
    super(...arguments);
    this.line = this.children.push(OrthogonalLine.new(this._root, { themeTags: ["line"] }));
    this.hitLine = this.children.push(OrthogonalLine.new(this._root, { themeTags: ["hit"] }));
    this.startArrow = this.children.push(Triangle.new(this._root, {
      themeTags: ["start", "arrow"]
    }));
    this.endArrow = this.children.push(Triangle.new(this._root, {
      themeTags: ["end", "arrow"]
    }));
  }
  _afterNew() {
    ...
    if (this.isDirty("points")) {
      ...
      if (len > 1) {
        ...
        startArrow.setAll({
          x: start.x,
          y: start.y,
          rotation: getAngle(start, next) - 90
        });
        ...
        endArrow.setAll({
          x: last.x,
          y: last.y,
          rotation: getAngle(prev, last) + 90
        });
      } else if (len === 1) {
        const [pt] = points;
...
Link.className = "Link";
Link.classNames = Container.classNames.concat([Link.className]);

// node_modules/@amcharts/amcharts5/.internal/core/render/Picture.js
var Picture = class extends Sprite {
  constructor() {
    super(...arguments);
    this._display = this._root._renderer.makePicture(void 0);
  }
  _changed() {
    super._changed();
...
Picture.classNames = Sprite.classNames.concat([Picture.className]);
...
RadialGradient.className = "RadialGradient";
RadialGradient.classNames = Gradient.classNames.concat([RadialGradient.className]);
    ...
    for (let r = rotation == 0 ? 0 : -rows * 2; r < rows * 2; r++) {
      for (let c = rotation == 0 ? 0 : -cols * 2; c < cols * 2; c++) {
        if (!checkered || (r & 1) != 1 && (c & 1) != 1 || (r & 1) == 1 && (c & 1) == 1) {
          ...
          display.drawRect(x2, y2, rectW, rectH);
        }
      }
    }
    if (checkered) {
...
PathPattern.className = "PathPattern";
PathPattern.classNames = Pattern.classNames.concat([PathPattern.className]);
...
var DataProcessor = class extends Entity {
  constructor() {
    super(...arguments);
    this._checkDates = false;
    this._checkNumbers = false;
    this._checkColors = false;
    this._checkEmpty = false;
    this._checkDeep = false;
  }
  _afterNew() {
    super._afterNew();
    this._checkFeatures();
...
var SpriteResizer = class extends Container {
  constructor() {
    super(...arguments);
    this.rectangle = this.children.push(Rectangle.new(this._root, { themeTags: ["rectangle"], fillOpacity: 0, fill: color(16777215) }));
    this.gripL = this._createGrip("left");
    this.gripR = this._createGrip("right");
    this.gripT = this._createGrip("top");
    this.gripB = this._createGrip("bottom");
    this._is = 1;
    this._ix = 0;
    this._iw = 0;
    this._isHover = false;
...
SpriteResizer.className = "SpriteResizer";
SpriteResizer.classNames = Container.classNames.concat([SpriteResizer.className]);

// node_modules/@amcharts/amcharts5/.internal/core/render/NumericStepper.js
var NumericStepper = class extends Container {
  constructor() {
    super(...arguments);
...
// node_modules/@amcharts/amcharts5/.internal/core/render/ProgressPie.js
var ProgressPie = class extends Container {
  constructor() {
    super(...arguments);
    this.circle = this.children.push(Circle.new(this._root, {
      themeTags: ["background"]
    }));
    this.label = this.children.push(Label.new(this._root, {}));
    this.slice = this.children.push(Slice.new(this._root, { isMeasured: false }));
    this.backgroundSlice = this.children.push(Slice.new(this._root, { themeTags: ["background"] }));
  }
  _afterNew() {
  ...
  _updateChildren() {
    ...
    if (this.isDirty("width") || this.isDirty("height") || this.isDirty("radius") || this.isDirty("innerRadius")) {
      ...
      const radius = relativeToValue(this.get("radius", p100), Math.min(width, height) / 2);
      const innerRadius = relativeToValue(this.get("innerRadius", percent(80)), radius);
      this.slice.setAll({
        radius,
        innerRadius
      });
...
ProgressPie.className = "ProgressPie";
ProgressPie.classNames = Container.classNames.concat([ProgressPie.className]);
