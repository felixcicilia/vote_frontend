...
MapSeries.className = "MapSeries";
MapSeries.classNames = Series.classNames.concat([MapSeries.className]);
...
MapLine.classNames = Graphics.classNames.concat([MapLine.className]);
...
var GraticuleSeries = class extends MapLineSeries {
  constructor() {
    super(...arguments);
    this._dataItem = this.makeDataItem({});
  }
  _afterNew() {
    super._afterNew();
...
GraticuleSeries.className = "GraticuleSeries";
GraticuleSeries.classNames = MapLineSeries.classNames.concat([GraticuleSeries.className]);
...
var MapChart = class extends SerialChart {
  constructor() {
    ...
    this._doubleDownDistance = 0;
    this._dirtyGeometries = false;
    this._geometryColection = { type: "GeometryCollection", geometries: [] };
    this._centerLocation = null;
    this._mapBounds = [[0, 0], [0, 0]];
    this._geoCentroid = { longitude: 0, latitude: 0 };
    this._geoBounds = { left: 0, right: 0, top: 0, bottom: 0 };
    this._prevGeoBounds = { left: 0, right: 0, top: 0, bottom: 0 };
  ...
  _prepareChildren() {
    ...
    if (this.isPrivateDirty("projectionBlend") && this._projectionBlendData) {
      ...
      if (t >= 1) {
        ...
        this._projectionBlendData = void 0;
        this._projectionBlendAnim = void 0;
  ...
  animateProjection(target, targetRaw, duration, easing, sourceRaw) {
    ...
    const tempTarget = projection(targetRaw);
    tempTarget.fitSize([w, h], this._geometryColection);
    const tScale = tempTarget.scale();
    const tTranslate = tempTarget.translate();
    const mutate = projectionMutator((t) => {
      return (lambda, phi2) => {
        const p02 = currentRaw(lambda, phi2);
        const p1 = targetRaw(lambda, phi2);
        return [
          (1 - t) * p02[0] + t * p1[0],
          (1 - t) * p02[1] + t * p1[1]
        ];
...
MapChart.className = "MapChart";
MapChart.classNames = SerialChart.classNames.concat([MapChart.className]);

// node_modules/@amcharts/amcharts5/.internal/charts/map/MapPointSeries.js
var MapPointSeries = class extends MapSeries {
  constructor() {
    super(...arguments);
    this._types = ["Point", "MultiPoint"];
  }
  _afterNew() {
...
MapPointSeries.className = "MapPointSeries";
MapPointSeries.classNames = MapSeries.classNames.concat([MapPointSeries.className]);
...
MapPolygon.className = "MapPolygon";
MapPolygon.classNames = Graphics.classNames.concat([MapPolygon.className]);
...
var MapPolygonSeries = class extends MapSeries {
  constructor() {
    ...
    this._types = ["Polygon", "MultiPolygon"];
  }
  /**
   * @ignore
   */
...
MapPolygonSeries.className = "MapPolygonSeries";
MapPolygonSeries.classNames = MapSeries.classNames.concat([MapPolygonSeries.className]);
...
var ClusteredPointSeries = class extends MapPointSeries {
  ...
  _clusterGroup(dataItems) {
    ...
    if (chart && chart.get("zoomLevel", 1) >= chart.get("maxZoomLevel", 100) * this.get("stopClusterZoom", 0.95)) {
      ...
      const candidates = [];
      for (let i2 = 0; i2 < dataItems.length; i2++) {
        const di = dataItems[i2];
        if (!di.get("clipped")) {
          const pt = di.get("point");
          if (pt) {
            candidates.push({ di, x: pt.x, y: pt.y });
          }
        }
      }
      ...
      for (let i2 = 0; i2 < candidates.length; i2++) {
        ...
        const key = Math.floor(c2.x / minDistance) + "," + Math.floor(c2.y / minDistance);
  ...
  _scatterGroup(dataItems) {
    ...
    if (chart && chart.get("zoomLevel", 1) >= chart.get("maxZoomLevel", 100) * this.get("stopClusterZoom", 0.95)) {
      ...
      for (let i = 0; i < candidates.length; i++) {
        ...
        const key = Math.floor(c2.x / scatterDistance) + "," + Math.floor(c2.y / scatterDistance);
        let cell = grid.get(key);
        if (!cell) {
          cell = [];
          grid.set(key, cell);
...
ClusteredPointSeries.className = "ClusteredPointSeries";
ClusteredPointSeries.classNames = MapPointSeries.classNames.concat([ClusteredPointSeries.className]);

// node_modules/@amcharts/amcharts5/.internal/charts/map/MapSankeyNodes.js
var MapSankeyNodes = class extends Series {
  constructor() {
    super(...arguments);
...
// node_modules/@amcharts/amcharts5/.internal/charts/map/MapSankeySeries.js
var MapSankeySeries = class extends MapPolygonSeries {
  constructor() {
    super(...arguments);
    this._types = [];
    this.nodes = this.addDisposer(MapSankeyNodes.new(this._root, {}));
    this._bezierSegments = /* @__PURE__ */ new Map();
    this._segmentCumulativeLengths = /* @__PURE__ */ new Map();
    this._geoSourceCoords = /* @__PURE__ */ new Map();
    this._geoTargetCoords = /* @__PURE__ */ new Map();
    this._geoWidths = /* @__PURE__ */ new Map();
    this._geoSourceGroups = /* @__PURE__ */ new Map();
    this._geoTargetGroups = /* @__PURE__ */ new Map();
    this._geoNodeSourceTotal = /* @__PURE__ */ new Map();
    this._geoNodeTargetTotal = /* @__PURE__ */ new Map();
    this._geoNodeRange = /* @__PURE__ */ new Map();
    this._geoSourceOffsets = /* @__PURE__ */ new Map();
    this._geoTargetOffsets = /* @__PURE__ */ new Map();
  }
  /**
   * @ignore
   */
  ...
  markDirtyProjection() {
    ...
    each(this.nodes.dataItems, (nodeDataItem) => {
  ...
  _segmentArcLength(bp, samples = 20) {
    ...
    for (let i = 1; i <= samples; i++) {
      ...
      const a2 = Math.pow(Math.sin(dLat / 2), 2) + Math.cos(prev.latitude * Math.PI / 180) * Math.cos(pt.latitude * Math.PI / 180) * Math.pow(Math.sin(dLon / 2), 2);
      length += 2 * Math.atan2(Math.sqrt(a2), Math.sqrt(1 - a2));
      prev = pt;
    }
    return length;
  }
   ...
   */
  getPoint(dataItem, location2) {
    const segments = this._bezierSegments.get(dataItem);
    const chart = this.chart;
    if (!segments || segments.length === 0 || !chart)
      return { x: 0, y: 0, angle: 0 };
    const { segIdx, t } = this._resolveLocation(dataItem, location2, segments);
    ...
    const angle2 = Math.atan2(sign2 * (screenPoint2.y - screenPoint.y), sign2 * (screenPoint2.x - screenPoint.x)) * 180 / Math.PI;
    return { x: screenPoint.x, y: screenPoint.y, angle: angle2 };
  }
  /**
   * Returns a geo point at a relative position (0-1) along the center-line bezier.
   */
  getGeoPoint(dataItem, location2) {
    const segments = this._bezierSegments.get(dataItem);
   ...
   * @ignore
   */
  _getPointWithGeo(dataItem, location2) {
    const segments = this._bezierSegments.get(dataItem);
    const chart = this.chart;
    if (!segments || segments.length === 0 || !chart)
      return void 0;
    ...
    const screenPoint2 = chart.convert({ longitude: pt2.longitude, latitude: pt2.latitude });
    const angle2 = Math.atan2(sign2 * (screenPoint2.y - screenPoint.y), sign2 * (screenPoint2.x - screenPoint.x)) * 180 / Math.PI;
    return { x: screenPoint.x, y: screenPoint.y, angle: angle2, geoPoint };
  }
  /**
  ...
  _positionBullet(bullet) {
    ...
    if (sprite) {
      const dataItem = sprite.dataItem;
      if (dataItem) {
        const location2 = bullet.get("locationX", 0.5);
        const result = this._getPointWithGeo(dataItem, location2);
        if (!result)
          return;
  ...
  _generateGeometries() {
    ...
    nodeRange.clear();
    const allKeys = /* @__PURE__ */ new Set([...nodeSourceTotal.keys(), ...nodeTargetTotal.keys()]);
    allKeys.forEach((key) => {
    ...
    sourceGroups.forEach((items, key) => {
      if (autoSort) {
        items.sort((a2, b) => {
          var _a, _b;
          return (((_a = targetCoords.get(a2)) === null || _a === void 0 ? void 0 : _a.latitude) || 0) - (((_b = targetCoords.get(b)) === null || _b === void 0 ? void 0 : _b.latitude) || 0);
        });
      }
      const ownTotal = nodeSourceTotal.get(key) || 0;
      const range3 = nodeRange.get(key) || ownTotal;
      let offset = -range3 / 2 + (range3 - ownTotal) / 2;
      each(items, (item) => {
    ...
    targetGroups.forEach((items, key) => {
      if (autoSort) {
        items.sort((a2, b) => {
          var _a, _b;
          return (((_a = sourceCoords.get(a2)) === null || _a === void 0 ? void 0 : _a.latitude) || 0) - (((_b = sourceCoords.get(b)) === null || _b === void 0 ? void 0 : _b.latitude) || 0);
        });
      }
    ...
    each(this.dataItems, (dataItem) => {
      ...
      for (let seg = 0; seg < pathPoints.length - 1; seg++) {
        ...
        if (seg === 0 && pathPoints.length === 2) {
          ...
          if (seg === 0) {
            tangent0Lon = p1.longitude - p02.longitude;
            tangent0Lat = p1.latitude - p02.latitude;
          } else {
          ...
          if (seg === pathPoints.length - 2) {
            tangent1Lon = p1.longitude - p02.longitude;
            tangent1Lat = p1.latitude - p02.latitude;
          } else {
            const next = pathPoints[seg + 2];
            tangent1Lon = next.longitude - p02.longitude;
            tangent1Lat = next.latitude - p02.latitude;
          }
        }
        const segDist = Math.sqrt(Math.pow(p1.longitude - p02.longitude, 2) + Math.pow(p1.latitude - p02.latitude, 2));
        ...
        const cp0Lon = p02.longitude + (t0Len > 0 ? tangent0Lon / t0Len * scale0 : 0);
        const cp0Lat = p02.latitude + (t0Len > 0 ? tangent0Lat / t0Len * scale0 : 0);
      ...
      if (area_default(geometry) > 2 * Math.PI) {
        ring.reverse();
        geometry = {
          type: "Polygon",
          coordinates: [ring]
      ...
      if (mapPolygon) {
        ...
        if (linkColorMode !== "solid") {
          const nodeDataItem = linkColorMode === "source" ? dataItem.get("sourceNode") : dataItem.get("targetNode");
          if (nodeDataItem) {
    ...
    each(this.nodes.dataItems, (nodeDataItem) => {
      ...
      let sumOutgoing = 0;
      each(outgoing, (link2) => {
        sumOutgoing += link2.get("value") || 0;
      });
      let sumIncoming = 0;
      each(incoming, (link2) => {
        sumIncoming += link2.get("value") || 0;
      });
      ...
      if (nodeType === "bar") {
        ...
        if (area_default(nodeGeo) > 2 * Math.PI) {
          ring.reverse();
          nodeGeo = { type: "Polygon", coordinates: [ring] };
        }
      } else {
...
ZoomControl.classNames = ZoomTools.classNames.concat([ZoomControl.className]);
export {
