...
export class PagarReservaComponent implements OnInit {
  ...
  selectedAccountId: string | null = null; // 'wallet' | 'tdc' | account.id (as string)
  ...
  get totalUsd(): number { return Number(this.booking?.totalPrice ?? 0); }
  get totalBs(): number {
  ...
  ngOnInit(): void {
    ...
    this.service.getBookingById(this.bookingId).subscribe({
      next: (b) => { this.booking = b; this.syncAmount(); this.loading = false; },
